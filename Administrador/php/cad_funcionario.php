...
if ($method === 'POST' && !isset($data['id'])) {
    $stmt = $conn->prepare("INSERT INTO Funcionario (NomeCompleto, Email, CPF, Telefone, Cargo, usuario, Senha) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssss", $data['NomeCompleto'], $data['Email'], $data['CPF'], $data['Telefone'], $data['Cargo'], $data['Usuario'], $data['Senha']);
    if ($stmt->execute()) {
...
// EDITAR
if ($method === 'POST' && isset($data['id'])) {
    $stmt = $conn->prepare("UPDATE Funcionario SET NomeCompleto=?, Email=?, CPF=?, Telefone=?, Cargo=?, Usuario=?, Senha=? WHERE id=?");
    $stmt->bind_param("sssssssi", $data['NomeCompleto'], $data['Email'], $data['CPF'], $data['Telefone'], $data['Cargo'], $data['Usuario'], $data['Senha'], $data['id']);
    if ($stmt->execute()) {
